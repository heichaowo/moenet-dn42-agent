...
protocol babel babel_igp {
    # Run on WG IGP mesh interface and dummy0 (for DN42 loopback)
    # Supports both old multi-interface (dn42-wg-igp-*) and new single interface (dn42-wg-igp)
    interface "dn42-wg-igp", "dn42-wg-igp-*", "wg-igp-*", "dummy0" {
        type wired;
        hello interval 4 s;
