...
from .constants import latency_to_tier
...
class LatencyProbe:
    ...
    async def _probe_all_peers(self) -> None:
        ...
        tasks = [self._probe_peer(peer) for peer in self.peers.values()]
            ...
            if isinstance(result, ProbeResult) and result.success:
                ...
                if peer:
                    ...
                    if peer.last_rtt is not None:
                        peer.last_rtt = (
                            self.ewma_alpha * result.rtt_ms + (1 - self.ewma_alpha) * peer.last_rtt
                        )
                    else:
                    ...
                    if old_tier != peer.last_tier and self._update_callback:
                        try:
                            self._update_callback(result.asn, peer.last_tier, peer.last_rtt)
                        except Exception as e:
    ...
    async def _ping(self, target: str) -> Optional[float]:
        ...
        try:
            ...
            stdout, _ = await asyncio.wait_for(
                proc.communicate(), timeout=self.timeout * self.probe_count + 5
            )
    ...
    def probe_now(self, asn: int) -> Optional[ProbeResult]:
        ...
        try:
            ...
            # Parse RTT
            match = re.search(
                r"rtt min/avg/max/mdev = [\d.]+/([\d.]+)/[\d.]+/[\d.]+ ms", result.stdout
            )
            if not match:
    ...
    def get_all_stats(self) -> dict:
        ...
        return {
            ...
            "peers": {asn: self.get_peer_stats(asn) for asn in self.peers},
        }
